...
namespace UE_PIE_Automation
{
	...
	int32 FPIEInputReplayer::FindSourceFrameForTimeline(double TimelineElapsedMs)
	{
		if (SourceFrames.Num() == 0) return INDEX_NONE;

		const double TargetTime = SourceFrames[0].Time + FMath::Max(0.0, TimelineElapsedMs) / 1000.0;
		while (SourceFrameCursor + 1 < SourceFrames.Num()
			&& SourceFrames[SourceFrameCursor + 1].Time <= TargetTime)
		{
			++SourceFrameCursor;
		}
	...
	void FPIEInputReplayer::TeleportPawnToStart(APawn* Pawn)
	{
		...
		UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-REP] Teleported pawn to frame 0: %s %s"),
	...
	void FPIEInputReplayer::EjectPlayer(UWorld* PIEWorld)
	{
		...
		if (Spectator)
		{
			...
			bEjected = true;
			UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-REP] Ejected to spectator"));
		}
	}
	...
	void FPIEInputReplayer::RepossessPlayer()
	{
		...
		if (PC && Pawn)
		{
			...
			UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-REP] Re-possessed original pawn"));
		}
		bEjected = false;
		EjectedPC.Reset();
		EjectedPawn.Reset();
	...
	bool FPIEInputReplayer::Arm(const FReplayerArmConfig& Cfg, FString& OutError, FString& OutMessage)
	{
		...
		else if (!Cfg.SourceRecordingId.IsEmpty())
		{
			...
			{
				// Non-fatal: disable source-frame features for this replay.
				UE_LOG(LogUE_PIE_Automation, Warning, TEXT("[PIE-REP] Source frame replay disabled (%s)"), *Err);
				SourceFrames.Reset();
			}
			if (Cfg.bRecordDrift && SourceFrames.Num() > 0)
			{
				...
				if (FPaths::FileExists(JsonlPath))
				{
					...
					{
						UE_LOG(LogUE_PIE_Automation, Warning, TEXT("[PIE-REP] tracked.jsonl read failed: %s"), *JErr);
						SourceActorRows.Reset();
					}
					else
					{
	...
	void FPIEInputReplayer::ApplyFPSPin(UWorld* PIEWorld, int32 Hz)
	{
		...
		UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-REP] %s"), *Cmd);
	...
	void FPIEInputReplayer::ApplyFixedTimestep(int32 Hz)
	{
		...
		UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-REP] Fixed timestep on: %.4f s (%d Hz)"), 1.0 / Hz, Hz);
	}
	...
	void FPIEInputReplayer::RestoreFixedTimestep()
	{
		...
		UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-REP] Fixed timestep restored"));
	}
	...
	void FPIEInputReplayer::OnBeginPIE(bool /*bIsSimulating*/)
	{
		...
		UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-REP] Armed → BeginPIE, waiting for pawn (%d steps)"), ActiveSequence.Steps.Num());
	}
	...
	void FPIEInputReplayer::ExecutePendingSteps(double ElapsedMs)
	{
		while (NextStepIndex < ActiveSequence.Steps.Num())
		{
			...
			case EStepType::Capture:
			{
				...
				UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-REP] capture requested: %s"), *FullPath);
				break;
			}
			}
			NextStepIndex++;
	...
	void FPIEInputReplayer::OnEndFrame()
	{
		...
		if (State == EReplayerState::Completed)
		{
			...
			if (Pending.bAutoStopPIE && !bEndPIERequested)
			{
				...
				if (bCaptureDrainComplete || bCaptureDrainTimedOut)
				{
					...
					{
						UE_LOG(LogUE_PIE_Automation, Log,
							TEXT("[PIE-REP] Capture drain complete (%d/%llu)"),
							FramesCaptured,
							CaptureFrameCounter);
					}
					if (GEditor)
					{
						UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-REP] Replay complete → auto-stopping PIE"));
			...
			if (Sampler.AttachToPIE(PIEWorld))
			{
				...
				if (Pending.TimeScale != 1.0f)
				{
					...
					if (WS)
					{
						...
						UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-REP] Time scale: %.2f"), Pending.TimeScale);
					}
				}
		...
		if (State == EReplayerState::Replaying)
		{
			...
			// preferred; CaptureFrameEvery remains as a legacy frame-index fallback.
			if ((Pending.CaptureFPS > 0 || Pending.CaptureFrameEvery > 0) && ViewportCapture.IsValid())
			{
				if (CaptureDir.IsEmpty())
				{
					...
					IFileManager::Get().FindFiles(OldFrames, *(CaptureDir / TEXT("frame_*.png")), true, false);
					for (const FString& OldFrame : OldFrames)
					{
						IFileManager::Get().Delete(*(CaptureDir / OldFrame), false, true, true);
					}
					IFileManager::Get().MakeDirectory(*CaptureDir, true);
				}
				FramesCaptured = ViewportCapture->GetCapturedCount();
				bool bShouldCapture = false;
				if (Pending.CaptureFPS > 0)
				{
					const double IntervalMs = 1000.0 / static_cast<double>(Pending.CaptureFPS);
					if (!bCaptureTimelineStarted)
					{
						bCaptureTimelineStarted = true;
						NextCaptureTimeMs = 0.0;
					}
					if (TimelineElapsedMs + 0.001 >= NextCaptureTimeMs)
					{
						bShouldCapture = true;
						do
						{
							NextCaptureTimeMs += IntervalMs;
						} while (NextCaptureTimeMs <= TimelineElapsedMs);
					}
				}
				else if ((ReplayFrame % static_cast<uint64>(Pending.CaptureFrameEvery)) == 0)
				{
					bShouldCapture = true;
				}
			...
			if (Pending.bRecordDrift && SourceFrames.Num() > 0)
			{
				...
				// ReplayFrameCounter can run a few extra fixed ticks because the
				// source CSV's first/last timestamps are not guaranteed to land on
				// the replay tick grid. Those are extra replay ticks, not missing
				// source frames, so do not report them as replay omissions.
			}

			// Complete when all steps/holds are done and the replay clock has
	...
	FReplayerFinishResult FPIEInputReplayer::FinaliseCurrent()
	{
		...
		if (SourceFrames.Num() > 0 && !CurrentDriftPath.IsEmpty())
		{
			...
			{
				UE_LOG(LogUE_PIE_Automation, Warning, TEXT("[PIE-REP] drift write failed: %s"), *Err);
			}
		}
		...
		if (!CaptureDir.IsEmpty() && FramesCaptured > 0)
		{
			...
			if (Frames.Num() > 0)
			{
				...
				// frame can be located visually. The source frames remain as PNGs;
				// no animated format is generated here.
