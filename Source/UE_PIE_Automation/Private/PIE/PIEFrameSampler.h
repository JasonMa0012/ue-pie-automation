...
namespace UE_PIE_Automation
{
	...
	public:
		...
		// computed against the previous frame's state. MousePosition is sampled
		// before this world's actors tick, so it represents the cursor state that
		// gameplay input handlers see before they can move the cursor themselves.
	...
	private:
		...
		FVector2D PreActorTickMousePosition = FVector2D::ZeroVector;
		bool bHasPreActorTickMousePosition = false;
	};
}
