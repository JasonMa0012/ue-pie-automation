...
namespace UE_PIE_Automation
{
	namespace
	{
		...
		FString DebugObjectPropertyName(APlayerController* PC, const TCHAR* Match)
		{
			if (!PC) return TEXT("<no-controller>");

			for (TFieldIterator<FProperty> It(PC->GetClass()); It; ++It)
			{
				FProperty* Property = *It;
				const FString NormalizedName = Property->GetName().Replace(TEXT("_"), TEXT(""));
				if (!NormalizedName.Contains(Match)) continue;
				if (NormalizedName.Contains(TEXT("ActorHoldingComponent"))) continue;

				if (FObjectPropertyBase* ObjectProperty = CastField<FObjectPropertyBase>(Property))
				{
					const void* ValueAddress = ObjectProperty->ContainerPtrToValuePtr<void>(PC);
					if (UObject* Value = ObjectProperty->GetObjectPropertyValue(ValueAddress))
					{
		...
		void LogMouseClickDebug(APlayerController* PC, uint64 FrameNumber,
			const FVector2D& MousePosition, bool bMousePositionValid, const FString& EdgeEvent)
		{
			const FString UnderCursor = DebugObjectPropertyName(PC, TEXT("ActorUnderCursor"));
			const FString Holding = DebugObjectPropertyName(PC, TEXT("ActorHolding"));
			UE_LOG(LogUE_PIE_Automation, Display,
				TEXT("[PIE-INPUT-DEBUG] edge=%s frame=%llu mouse_valid=%d mouse=(%.0f,%.0f) under=%s holding=%s"),
				*EdgeEvent,
				FrameNumber,
				bMousePositionValid ? 1 : 0,
				MousePosition.X,
				MousePosition.Y,
				*UnderCursor,
				*Holding);
	...
	FPIEFrameSampler::~FPIEFrameSampler()
	{
	...
	void FPIEFrameSampler::Reset()
	{
		...
		PreActorTickMousePosition = FVector2D::ZeroVector;
	...
	bool FPIEFrameSampler::AttachToPIE(UWorld* PIEWorld)
	{
		...
		AttachedWorld = PIEWorld;
		bAttached = true;
		WorldPreActorTickHandle = FWorldDelegates::OnWorldPreActorTick.AddRaw(
			this, &FPIEFrameSampler::OnWorldPreActorTick);
		UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-SAMPLER] Attached: %d actions, %d tracked values, pawn=%s"),
			Actions.Num(), TrackedValues.Num(), *PawnClassPath);
		return true;
	}

	void FPIEFrameSampler::OnWorldPreActorTick(UWorld* World, ELevelTick /*TickType*/, float /*DeltaSeconds*/)
	{
		if (!bAttached || World != AttachedWorld.Get()) return;

		bHasPreActorTickMousePosition = false;
		APlayerController* PC = (Config.ClientIndex > 0)
			? UGameplayStatics::GetPlayerController(World, Config.ClientIndex)
		...
		{
			PreActorTickMousePosition = FVector2D(MouseX, MouseY);
			bHasPreActorTickMousePosition = true;
		}
	}

	void FPIEFrameSampler::UnbindWorldPreActorTick()
	{
	...
	FCSVRow FPIEFrameSampler::SampleFrame(UWorld* PIEWorld, uint64 FrameNumber, double GameTime, double DeltaTime)
	{
		...
		// Capture the cursor before this world's actors tick. Enhanced Input is
		// processed from PlayerController::PlayerTick before the controller's
		// Blueprint Tick, so this is the position the current input event saw.
		// This prevents a SetMousePosition call made by that same event from
		// being replayed before the event that caused it.
		if (bHasPreActorTickMousePosition)
		{
			Row.MousePosition = PreActorTickMousePosition;
			Row.bMousePositionValid = true;
			bHasPreActorTickMousePosition = false;
		}
		else
		{
			// Attach can happen at the end of a frame, before the first
			// pre-actor callback. Keep the first row usable with a direct read.
			float MouseX = 0.f;
			float MouseY = 0.f;
			Row.bMousePositionValid = PC->GetMousePosition(MouseX, MouseY);
			if (Row.bMousePositionValid)
			{
				Row.MousePosition = FVector2D(MouseX, MouseY);
			}
		}
		...
		// Rescan for late-bound actions (IMCs added after initial attach).
		{
			...
			{
				UE_LOG(LogUE_PIE_Automation, Log, TEXT("[PIE-SAMPLER] Late-discovered action: %s (%s)"),
		...
		for (const FString& EdgeEvent : Row.EdgeEvents)
		{
			if (EdgeEvent.Contains(TEXT("MouseLeftClick")))
			{
				LogMouseClickDebug(PC, FrameNumber, Row.MousePosition,
					Row.bMousePositionValid, EdgeEvent);
			}
		}
