...
namespace UE_PIE_Automation
{
	...
	struct FReplayerArmConfig
	{
		...
		// Drive the engine at a fixed delta time (1/pin_fps) during replay instead
		// of only capping FPS. This is the default because input timing and PNG
		// capture indices must be repeatable; callers can opt out explicitly.
		// It improves reproducibility but cannot make Chaos/async rendering fully
		// deterministic. Restored on finish.
	...
	struct FReplayerFinishResult
	{
		...
		// per-frame PNGs (frame_NNNNN.png); ContactSheetPath is a single grid
	...
	private:
		...
		double NextCaptureTimeMs = 0.0;
		bool bCaptureTimelineStarted = false;
		FString CaptureDir;
