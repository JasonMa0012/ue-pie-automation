...
namespace UE_PIE_Automation
{
	namespace
	{
		...
		bool LoadImagePixels(const FString& Path, TArray<FColor>& OutPixels, int32& OutW, int32& OutH)
		{
			...
			const EImageFormat Format = IWM.DetectImageFormat(FileData.GetData(), FileData.Num());
		...
		const FGifEncodeParams& Params)
	{
		...
		int32 SrcW = 0, SrcH = 0;
		if (!LoadImagePixels(FramePaths[0], FirstPixels, SrcW, SrcH))
		{
			UE_LOG(LogUE_PIE_Automation, Warning, TEXT("[GIF] Failed to load first frame: %s"), *FramePaths[0]);
			return false;
		}
		...
		for (int32 i = 1; i < FramePaths.Num(); i++)
		{
			...
			int32 W = 0, H = 0;
			if (!LoadImagePixels(FramePaths[i], Pixels, W, H))
			{
				UE_LOG(LogUE_PIE_Automation, Warning, TEXT("[GIF] Skipping unreadable frame: %s"), *FramePaths[i]);
				continue;
			}
		...
		{
			UE_LOG(LogUE_PIE_Automation, Warning, TEXT("[GIF] Failed to write: %s"), *OutputPath);
			return false;
		}

		UE_LOG(LogUE_PIE_Automation, Log, TEXT("[GIF] Wrote %d frames -> %s (%.1f KB)"),
