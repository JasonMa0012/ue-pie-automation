...
namespace
{
	...
	{
		return UE_PIE_Automation::FPIEPredicateEvaluator::DeriveStarterPredicates(CsvPath, MaxErrors);
	}
}
...
TSharedPtr<FJsonValue> FGameplayHandlers::PieTestRun(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		TArray<UE_PIE_Automation::FPredicate> Preds;
		FString PErr;
		if (!UE_PIE_Automation::FPIEPredicateEvaluator::ParsePredicates(*PredArr, Preds, PErr))
		{
			return MCPError(FString::Printf(TEXT("predicate parse error: %s"), *PErr));
		}
		TArray<UE_PIE_Automation::FPredicateResult> PResults;
		if (UE_PIE_Automation::FPIEPredicateEvaluator::Evaluate(
				...
				Source / TEXT("manifest.json"), Preds, PResults, PErr))
		{
			...
			TSharedRef<FJsonObject> PBlock = UE_PIE_Automation::FPIEPredicateEvaluator::ResultsToJson(PResults, bPredPassed);
...
TSharedPtr<FJsonValue> FGameplayHandlers::PieAssertEval(const TSharedPtr<FJsonObject>& Params)
{
	...
	TArray<UE_PIE_Automation::FPredicate> Preds;
	FString Err;
	if (!UE_PIE_Automation::FPIEPredicateEvaluator::ParsePredicates(*AArr, Preds, Err))
	{
		return MCPError(FString::Printf(TEXT("predicate parse error: %s"), *Err));
	}

	TArray<UE_PIE_Automation::FPredicateResult> Results;
	if (!UE_PIE_Automation::FPIEPredicateEvaluator::Evaluate(
	...
	bool bPassed = true;
	TSharedRef<FJsonObject> Verdict = UE_PIE_Automation::FPIEPredicateEvaluator::ResultsToJson(Results, bPassed);
