...
namespace
{
	using namespace UEPIEAutomation;
	...
	{
		return UEPIEAutomation::FPIEPredicateEvaluator::DeriveStarterPredicates(CsvPath, MaxErrors);
	}
}
...
TSharedPtr<FJsonValue> FGameplayHandlers::PieTestRun(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		TArray<UEPIEAutomation::FPredicate> Preds;
		FString PErr;
		if (!UEPIEAutomation::FPIEPredicateEvaluator::ParsePredicates(*PredArr, Preds, PErr))
		{
			return MCPError(FString::Printf(TEXT("predicate parse error: %s"), *PErr));
		}
		TArray<UEPIEAutomation::FPredicateResult> PResults;
		if (UEPIEAutomation::FPIEPredicateEvaluator::Evaluate(
				...
				Source / TEXT("manifest.json"), Preds, PResults, PErr))
		{
			...
			TSharedRef<FJsonObject> PBlock = UEPIEAutomation::FPIEPredicateEvaluator::ResultsToJson(PResults, bPredPassed);
...
TSharedPtr<FJsonValue> FGameplayHandlers::PieAssertEval(const TSharedPtr<FJsonObject>& Params)
{
	...
	TArray<UEPIEAutomation::FPredicate> Preds;
	FString Err;
	if (!UEPIEAutomation::FPIEPredicateEvaluator::ParsePredicates(*AArr, Preds, Err))
	{
		return MCPError(FString::Printf(TEXT("predicate parse error: %s"), *Err));
	}

	TArray<UEPIEAutomation::FPredicateResult> Results;
	if (!UEPIEAutomation::FPIEPredicateEvaluator::Evaluate(
	...
	bool bPassed = true;
	TSharedRef<FJsonObject> Verdict = UEPIEAutomation::FPIEPredicateEvaluator::ResultsToJson(Results, bPassed);
