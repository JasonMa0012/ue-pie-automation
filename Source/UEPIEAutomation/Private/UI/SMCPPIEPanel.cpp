...
#define IMAGE_BRUSH(RelativePath, ...) FSlateImageBrush(UEPIEAutomationStyleSet->RootToContentDir(RelativePath, TEXT(".png")), __VA_ARGS__)

static TSharedPtr<FSlateStyleSet> UEPIEAutomationStyleSet;

static void RegisterUEPIEAutomationStyle()
{
	if (UEPIEAutomationStyleSet.IsValid()) return;

	FString ResourcesDir = FPaths::Combine(
		FPaths::ProjectPluginsDir(), TEXT("UEPIEAutomation"), TEXT("Resources"));

	UEPIEAutomationStyleSet = MakeShareable(new FSlateStyleSet("UEPIEAutomationStyle"));
	UEPIEAutomationStyleSet->SetContentRoot(ResourcesDir);
	UEPIEAutomationStyleSet->Set("UEPIEAutomation.Record", new IMAGE_BRUSH("Record_Icon40x", CoreStyleConstants::Icon16x16));
	UEPIEAutomationStyleSet->Set("UEPIEAutomation.RecordPlay", new IMAGE_BRUSH("RecordPlay_Icon40x", CoreStyleConstants::Icon16x16));
	UEPIEAutomationStyleSet->Set("UEPIEAutomation.Play", new IMAGE_BRUSH("Play_Icon40x", CoreStyleConstants::Icon16x16));
	FSlateStyleRegistry::RegisterSlateStyle(*UEPIEAutomationStyleSet);
}

static void UnregisterUEPIEAutomationStyle()
{
	if (UEPIEAutomationStyleSet.IsValid())
	{
		FSlateStyleRegistry::UnRegisterSlateStyle(*UEPIEAutomationStyleSet);
		UEPIEAutomationStyleSet.Reset();
	}
}
...
namespace
{
	FString RecorderStateStr(UEPIEAutomation::ERecorderState S)
	{
		switch (S)
		{
		case UEPIEAutomation::ERecorderState::Idle:           return TEXT("Idle");
		case UEPIEAutomation::ERecorderState::Armed:          return TEXT("Armed");
		case UEPIEAutomation::ERecorderState::WaitingForPawn: return TEXT("Waiting for Pawn");
		case UEPIEAutomation::ERecorderState::Recording:      return TEXT("Recording");
		}
		return TEXT("?");
	}

	FString ReplayerStateStr(UEPIEAutomation::EReplayerState S)
	{
		switch (S)
		{
		case UEPIEAutomation::EReplayerState::Idle:           return TEXT("Idle");
		case UEPIEAutomation::EReplayerState::Armed:          return TEXT("Armed");
		case UEPIEAutomation::EReplayerState::WaitingForPawn: return TEXT("Waiting for Pawn");
		case UEPIEAutomation::EReplayerState::Replaying:      return TEXT("Replaying");
		case UEPIEAutomation::EReplayerState::Completed:      return TEXT("Completed");
		}
		return TEXT("?");
	}

	FString ObserverStateStr(UEPIEAutomation::EObserverState S)
	{
		switch (S)
		{
		case UEPIEAutomation::EObserverState::Idle:           return TEXT("Idle");
		case UEPIEAutomation::EObserverState::Armed:          return TEXT("Armed");
		case UEPIEAutomation::EObserverState::WaitingForPawn: return TEXT("Waiting for Pawn");
		case UEPIEAutomation::EObserverState::Observing:      return TEXT("Observing");
		case UEPIEAutomation::EObserverState::Completed:      return TEXT("Completed");
		}
		return TEXT("?");
	}
...
{
	RegisterUEPIEAutomationStyle();

	UToolMenu* ToolBar = UToolMenus::Get()->ExtendMenu("LevelEditor.LevelEditorToolBar.PlayToolBar");
	FToolMenuSection& Section = ToolBar->FindOrAddSection("UEPIEAutomation");

	Section.AddDynamicEntry("UEPIEAutomationActions", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
	{
		{
			FToolMenuEntry Entry =
				FToolMenuEntry::InitToolBarButton(
					...
					{
						UEPIEAutomation::FRecorderArmConfig Cfg;
						FString Err, Msg;
						UEPIEAutomation::FPIEInputRecorder::Get().Arm(Cfg, Err, Msg);
					}),
					FText::GetEmpty(),
					FText::FromString(TEXT("Arm MCP recorder (waits for PIE start)")),
					FSlateIcon("UEPIEAutomationStyle", "UEPIEAutomation.Record"));
...

		{
			FToolMenuEntry Entry =
				FToolMenuEntry::InitToolBarButton(
					...
					FSlateIcon("UEPIEAutomationStyle", "UEPIEAutomation.RecordPlay"));
			...
			FToolMenuEntry ComboEntry =
				FToolMenuEntry::InitComboButton(
					...
					FNewMenuDelegate::CreateLambda([](FMenuBuilder& Menu)
					{
						...
						const auto RecState = UEPIEAutomation::FPIEInputRecorder::Get().GetStatus().State;

						if (RecState == UEPIEAutomation::ERecorderState::Armed || RecState == UEPIEAutomation::ERecorderState::WaitingForPawn)
						{
							Menu.AddMenuEntry(
								...
								{
									FString Err;
									UEPIEAutomation::FPIEInputRecorder::Get().Disarm(Err);
								}))
							);
						}

						if (RecState == UEPIEAutomation::ERecorderState::Recording)
						{
							Menu.AddMenuEntry(
								...
								FUIAction(FExecuteAction::CreateLambda([]()
								{
									UEPIEAutomation::FPIEInputRecorder::Get().ForceStop();
								}))
							);
						}
...
{
	UnregisterUEPIEAutomationStyle();
}
	...
	return SNew(SVerticalBox)
		...
		[
			...
			[
				...
				.OnClicked_Lambda([this]()
				{
					FString PackagePath = TEXT("/Game/UEPIEAutomation");
...
void SMCPPIEPanel::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	// Update state labels
	{
		const auto RS = UEPIEAutomation::FPIEInputRecorder::Get().GetStatus();
		const FString RecText = FString::Printf(TEXT("%s  %s  F:%d  %.1fs"),
			*RecorderStateStr(RS.State), *RS.Id, RS.CurrentFrame, RS.ElapsedSeconds);
		const bool bRecActive = RS.State == UEPIEAutomation::ERecorderState::Recording;
	...
	{
		const bool bReplayActive = UEPIEAutomation::FPIEInputReplayer::Get().IsActive();
		const bool bObserverActive = UEPIEAutomation::FPIEObserver::Get().IsActive();
...
void SMCPPIEPanel::RefreshRecordings()
{
	...
	const FString Root = UEPIEAutomation::DefaultRecordingsRoot();
	TArray<FString> Dirs;
		...
		[
			...
			[
				...
				[
					...
					{
						UEPIEAutomation::FReplayerArmConfig Cfg;
						Cfg.SourceRecordingId = Id;
						Cfg.CaptureFrameEvery = bCaptureGifOnReplay ? 2 : 0;
						FString Err, Msg;
						UEPIEAutomation::FPIEInputReplayer::Get().Arm(Cfg, Err, Msg);
						for (const FString& ProfilePath : ActiveProfilePaths)
						{
							UEPIEAutomation::FObserverArmConfig OCfg;
							OCfg.ProfilePath = ProfilePath;
							FString OErr, OMsg;
							UEPIEAutomation::FPIEObserver::Get().Arm(OCfg, OErr, OMsg);
						}
						if (GEditor && !GEditor->PlayWorld)
						{
							FRequestPlaySessionParams P;
					...
					[
						SNew(SImage)
						.Image(FSlateStyleRegistry::FindSlateStyle("UEPIEAutomationStyle")->GetBrush("UEPIEAutomation.Play"))
				...
				[
					...
					.OnClicked_Lambda([this, Id]()
					{
						const FString RecPath = UEPIEAutomation::DefaultRecordingsRoot() / Id;
	...
	return SNew(SExpandableArea)
		...
		[
			...
			[
				...
				.Text_Lambda([]() -> FText
				{
					const bool bRep = UEPIEAutomation::FPIEInputReplayer::Get().IsActive();
					const bool bObs = UEPIEAutomation::FPIEObserver::Get().IsActive();
...
void SMCPPIEPanel::RefreshLiveDebug()
{
	...
	// Replay status
	const auto Rep = UEPIEAutomation::FPIEInputReplayer::Get().GetLiveSnapshot();
	if (Rep.State == UEPIEAutomation::EReplayerState::Replaying)
	{
	...
	TArray<UEPIEAutomation::FLiveObservationSnapshot> Snaps = UEPIEAutomation::FPIEObserver::Get().GetLiveSnapshots();
	...
	if (Rep.State != UEPIEAutomation::EReplayerState::Replaying && Snaps.Num() == 0)
	{
		LiveDebugContent->AddSlot().AutoHeight()
		[
			SNew(STextBlock)
