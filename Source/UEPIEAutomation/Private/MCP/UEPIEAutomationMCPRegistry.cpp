#include "UEPIEAutomationMCPRegistry.h"
...
namespace
{
	using FHandler = FUEPIEAutomationMCPTool::FHandler;
	...
	FString SchemaPath()
	{
		if (const TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("UEPIEAutomation")))
		{
			return Plugin->GetBaseDir() / TEXT("Resources/UEPIEAutomationTools.json");
		}
		return FPaths::ProjectPluginsDir() / TEXT("UEPIEAutomation/Resources/UEPIEAutomationTools.json");
	}
}

bool FUEPIEAutomationMCPRegistry::Register()
{
	...
	RefreshHandle = Module->OnRefreshTools().AddRaw(this, &FUEPIEAutomationMCPRegistry::RegisterTools);
	RegisterTools();
	return Tools.Num() > 0;
}

void FUEPIEAutomationMCPRegistry::Unregister()
{
...
void FUEPIEAutomationMCPRegistry::RegisterTools()
{
	IModelContextProtocolModule* Module = FModuleManager::GetModulePtr<IModelContextProtocolModule>(TEXT("ModelContextProtocol"));
	if (!Module) return;
	...
	for (const TSharedPtr<FJsonValue>& Value : *ToolValues)
	{
		...
		TSharedRef<FUEPIEAutomationMCPTool> Tool = MakeShared<FUEPIEAutomationMCPTool>(Name, Description, InputSchema, Handler);
