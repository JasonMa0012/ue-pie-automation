...
namespace UEPIEAutomation
{
		...
		const FGifEncodeParams& Params)
	{
		...
		{
			UE_LOG(LogUEPIEAutomation, Warning, TEXT("[GIF] Failed to load first frame: %s"), *FramePaths[0]);
			return false;
		}
		...
		for (int32 i = 1; i < FramePaths.Num(); i++)
		{
			...
			{
				UE_LOG(LogUEPIEAutomation, Warning, TEXT("[GIF] Skipping unreadable frame: %s"), *FramePaths[i]);
				continue;
			}
		...
		{
			UE_LOG(LogUEPIEAutomation, Warning, TEXT("[GIF] Failed to write: %s"), *OutputPath);
			return false;
		}

		UE_LOG(LogUEPIEAutomation, Log, TEXT("[GIF] Wrote %d frames -> %s (%.1f KB)"),
