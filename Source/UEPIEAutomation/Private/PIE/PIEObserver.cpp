...
namespace UEPIEAutomation
{
	...
	void FPIEObserver::OnBeginPIE(bool /*bIsSimulating*/)
	{
		...
		for (FObservationSession& S : Sessions)
		{
			...
			UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-OBS] Armed -> BeginPIE: profile=%s run=%s"),
		...
		for (FObservationSession& S : Sessions)
		{
			if (S.State == EObserverState::WaitingForPawn)
			{
				if (S.Sampler.AttachToPIE(PIEWorld))
				{
					...
					UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-OBS] Pawn attached, observing: %s"), *S.RunId);
				}
				continue;
			}
	...
	FObserverFinishResult FPIEObserver::FinaliseSession(FObservationSession& S)
	{
...

		{
			...
			{
				UE_LOG(LogUEPIEAutomation, Warning, TEXT("[PIE-OBS] CSV write failed: %s"), *Err);
			}
		}
		...
		if (S.ActorRows.Num() > 0)
		{
			...
			{
				UE_LOG(LogUEPIEAutomation, Warning, TEXT("[PIE-OBS] tracked.jsonl write failed: %s"), *Err);
			}
		}
		...
		UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-OBS] Finalized: %s %d frames -> %s"),
