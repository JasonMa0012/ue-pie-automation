...
namespace UEPIEAutomation
{
	namespace
	{
		...
		// Sample one channel of a named bone or socket
		// transform per frame. A point-in-time
	...
	bool FPIEFrameSampler::AttachToPIE(UWorld* PIEWorld)
	{
		...
		bAttached = true;
		UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-SAMPLER] Attached: %d actions, %d tracked values, pawn=%s"),
	...
	FCSVRow FPIEFrameSampler::SampleFrame(UWorld* PIEWorld, uint64 FrameNumber, double GameTime, double DeltaTime)
	{
		...
		// Rescan for late-bound actions (IMCs added after initial attach).
		{
			...
			{
				UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-SAMPLER] Late-discovered action: %s (%s)"),
