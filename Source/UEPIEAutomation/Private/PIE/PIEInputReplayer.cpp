...
namespace UEPIEAutomation
{
	...
	void FPIEInputReplayer::TeleportPawnToStart(APawn* Pawn)
	{
		...
		UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-REP] Teleported pawn to frame 0: %s %s"),
	...
	void FPIEInputReplayer::EjectPlayer(UWorld* PIEWorld)
	{
		...
		if (Spectator)
		{
			...
			bEjected = true;
			UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-REP] Ejected to spectator"));
		}
	}
	...
	void FPIEInputReplayer::RepossessPlayer()
	{
		...
		if (PC && Pawn)
		{
			...
			UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-REP] Re-possessed original pawn"));
		}
		bEjected = false;
		EjectedPC.Reset();
		EjectedPawn.Reset();
		...
		else if (!Cfg.SourceRecordingId.IsEmpty())
		{
			...
			if (Cfg.bRecordDrift)
			{
				...
				if (!LoadSourceFrames(CurrentSourceCSV, Err))
				{
					...
					UE_LOG(LogUEPIEAutomation, Warning, TEXT("[PIE-REP] Drift disabled (%s)"), *Err);
					SourceFrames.Reset();
				}
				...
				if (FPaths::FileExists(JsonlPath))
				{
					...
					{
						UE_LOG(LogUEPIEAutomation, Warning, TEXT("[PIE-REP] tracked.jsonl read failed: %s"), *JErr);
						SourceActorRows.Reset();
					}
					else
					{
	...
	void FPIEInputReplayer::ApplyFPSPin(UWorld* PIEWorld, int32 Hz)
	{
		...
		UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-REP] %s"), *Cmd);
	...
	void FPIEInputReplayer::ApplyFixedTimestep(int32 Hz)
	{
		...
		UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-REP] Fixed timestep on: %.4f s (%d Hz)"), 1.0 / Hz, Hz);
	}
	...
	void FPIEInputReplayer::RestoreFixedTimestep()
	{
		...
		UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-REP] Fixed timestep restored"));
	}
	...
	void FPIEInputReplayer::OnBeginPIE(bool /*bIsSimulating*/)
	{
		...
		UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-REP] Armed → BeginPIE, waiting for pawn (%d steps)"), ActiveSequence.Steps.Num());
	}
	...
	void FPIEInputReplayer::ExecutePendingSteps(double ElapsedMs)
	{
		while (NextStepIndex < ActiveSequence.Steps.Num())
		{
			...
			{
				if (UInputAction* Act = LoadAction(S.Action))
				{
					FString Err;
			...
			case EStepType::Capture:
			{
				...
				UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-REP] capture requested: %s"), *FullPath);
				break;
			}
			}
			NextStepIndex++;
	...
	void FPIEInputReplayer::OnEndFrame()
	{
			...
			if (Sampler.AttachToPIE(PIEWorld))
			{
				...
				if (Pending.TimeScale != 1.0f)
				{
					...
					if (WS)
					{
						...
						UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-REP] Time scale: %.2f"), Pending.TimeScale);
					}
				}
		...
		if (State == EReplayerState::Completed && Pending.bAutoStopPIE && !bEndPIERequested)
		{
			...
			if (GEditor)
			{
				UE_LOG(LogUEPIEAutomation, Log, TEXT("[PIE-REP] Replay complete → auto-stopping PIE"));
	...
	FReplayerFinishResult FPIEInputReplayer::FinaliseCurrent()
	{
		...
		if (SourceFrames.Num() > 0 && !CurrentDriftPath.IsEmpty())
		{
			...
			{
				UE_LOG(LogUEPIEAutomation, Warning, TEXT("[PIE-REP] drift write failed: %s"), *Err);
			}
		}
